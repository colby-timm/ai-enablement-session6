...
2. **Given** no overdue todos exist, **When** the user views the list, **Then** the overdue count shows zero or is not displayed
3. **Given** the user completes an overdue todo, **When** the list refreshes, **Then** the overdue count decrements by one
...
- What happens at midnight when today's date changes? (Previously current todos should become overdue if incomplete)
...
- **FR-008**: System MUST update overdue status dynamically as dates change (e.g., at midnight transitions)

### Visual Design Requirements

- **FR-009**: Overdue visual indicators MUST be clearly distinguishable from normal todo styling through one or more of: color change, icon addition, text label, or border styling
- **FR-010**: Overdue visual styling MUST maintain readability and not obscure todo content
- **FR-011**: Overdue indicators MUST be consistent with the application's existing design system (light/dark mode support, color palette, Halloween theme)
...
- Server time is the authoritative source for "today's date" to handle timezone and clock discrepancies
